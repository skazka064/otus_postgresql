...
#### уменьшили effective_cache_size, random_page_cost
...
* work_mem
  - Этот параметр определяет объем памяти, который может использоваться каждым запросом (не сеансом) для внутренних операций сортировки (таких как ORDER BY и DISTINCT) и хэш-таблиц (например, при выполнении агрегации на основе хэша). Помимо этого, PostgreSQL перемещает данные во временные файлы на диске. Обычно проблема заключается в том, чтобы найти здесь хороший баланс. Мы хотим избежать использования временных файлов на диске, которые замедляют выполнение запроса и, в свою очередь, могут вызывать конфликты. Но мы не хотим перераспределять память, что может даже привести к OOM; работа с высокими значениями work_mem может быть разрушительной, когда она на самом деле не нужна. Мы в дальнейшем попробуем поиграть с этим параметром и посмотреть когда у нас данные начнут уходить в своп.
 * random_page_cost
   - Этот параметр определяет стоимость, которую будет иметь непоследовательно извлеченная страница диска, и напрямую влияет на решения планировщика запросов. Использование консервативного значения особенно важно при использовании хранилища с высокой задержкой, например, вращающихся дисков. Это не наш случай, поэтому мы могли позволить себе уравнять random_page_cost с seq_page_cost . Поэтому мы также устанавливаем этот параметр на 1.1, что ниже значения по умолчанию 4.
  * effective_io_concurrency
    - Задаёт допустимое число параллельных операций ввода/вывода, которое говорит PostgreSQL о том, сколько операций ввода/вывода могут быть выполнены одновременно. Чем больше это число, тем больше операций ввода/вывода будет пытаться выполнить параллельно PostgreSQL в отдельном сеансе. Допустимые значения лежат в интервале от 1 до 1000, а нулевое значение отключает асинхронные запросы ввода/вывода. В настоящее время этот параметр влияет только на сканирование по битовой карте.
<table>
 <th>Num</th>
...
```
* Вывод_1
  - Т.к. нагрузка у нас ближе к oltp(web), то соответственно ее параметры подошли больше всего
  - Поэтому возьмем ее за основу, и произведем более тонкие настройки, с помощью sysbench-tpcc
...
#### Попробуем накрутить work_mem до 1GB + предыдущие WEB настройки(посмотрим загрузку железа)
...
#### Сбросим настройки до конфигурации WEB и установим effective_io_concurrency в 1000, т.е. в максимум
![Иллюстрация к проекту](img/2025-01-09_10-08-32.png)
...
```
### статистика работы нагрузки с настройками WEB + 1GB work_mem
```bash
SQL statistics:
    queries performed:
        read:                            778806
...
```
#### с work_mem=1GB результат улучшился. однако следует заметить, что max_connections=100, а мы запускали тест с 90 пользователями, поэтому памяти хватило. 
...
* Вывод_2:
  ...
  -  Уменьшились TPS.
  -  И на графике загрузки железа можно увидеть незначительное использование файла подкачки (0.3%), и увеличение загрузки RAM.
...
### Теперь возьмем настройку, при которой у нас получились самые лучшие рез-ты (WEB+work_mem=1GB) и добавим побольше shared_buffers=10G 
```bash
SQL statistics:
    queries performed:
        read:                            630230
        ...
        other:                           145420
        total:                           1418559
    transactions:                        40729  (336.92 per sec.)
    queries:                             1418559 (11734.76 per sec.)
    ignored errors:                      32139  (265.86 per sec.)
    reconnects:                          0      (0.00 per sec.)

General statistics:
    total time:                          120.8836s
    total number of events:              40729
...
Threads fairness:
    events (avg/stddev):           452.5444/25.24
    execution time (avg/stddev):   120.2679/0.39
```
* Вывод_4
  - результаты немного ухудшились. shared_buffers -определяет объем памяти, который PostgreSQL использует для буферов разделяемой памяти. Т.к. у нас в ВМ всего 4G то естественно паняти для всех процессов перестало хватать.         
 work_mem=90*1G и добавился увеличенный shared_buffers=10G
#### shared_buffers=1G поставил 25% от всей памяти ; 
#### effective_cache_size = 2GB - более высокое значение делает более вероятным использование сканирования индекса, более низкое значение делает более вероятным использование последовательного сканирования. Также следует учитывать ожидаемое количество параллельных запросов к разным таблицам, поскольку им придется совместно использовать доступное пространство. Этот параметр не влияет на размер общей памяти, выделяемой PostgreSQL , и не резервирует дисковый кэш ядра; он используется только для оценки. Сообщает планировщику запросов PostgreSQL, какой объем оперативной памяти, по оценкам, доступен для кэширования данных, как в shared_buffers, так и в кэше файловой системы. Этот параметр просто помогает планировщику делать хорошие оценки затрат; он фактически не выделяет память. Рекомедуется поставить 50 % от всей памяти ОЗУ; 
#### Решил увеличить work_mem = 10MB т.к. я предположил, что в этих скриптах имеются запросы с  сортировками но сильно его увеличивать тоже не рекомендуется, т.к. при увеличении количества пользователей возрастет нагрузка на память ; 
#### maintenance_work_mem = 205MB это параметр памяти, используемый для задач обслуживания. Значение по умолчанию составляет 64 МБ. Установка большого значения помогает в таких задачах, как VACUUM, RESTORE, CREATE INDEX, ADD FOREIGN KEY и ALTER TABLE. я его увеличил, т.к., например при операциях update будет активно использоваться vacuum, что бы он успевал отрабатывать. 
```bash
SQL statistics:
    queries performed:
        read:                            681173
        ...
        other:                           108528
        total:                           1495844
    transactions:                        51878  (432.22 per sec.)
    queries:                             1495844 (12462.69 per sec.)
    ignored errors:                      2602   (21.68 per sec.)
    reconnects:                          0      (0.00 per sec.)

General statistics:
    total time:                          120.0231s
    total number of events:              51878
...
Threads fairness:
    events (avg/stddev):           12969.5000/114.30
    execution time (avg/stddev):   119.9661/0.01
```
![Иллюстрация к проекту](img/2025-01-10_11-43-21.png)

* Вывод
  - С этими натройками мы добились максимальной производительности и более экономного расходования ресурсов сервера.
