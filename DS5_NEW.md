...
* work_mem
  - мы создали оптимальный параметр , т.к. слишком маленьким его не стоит делать, как и слишком большим. на сервере может быть много сессий, а каждая сессия будет использовать свой work_mem и если количество пользователей большое, то и work_mem будет большое и памяти может не хватить. так же как и большое количество операций сортировок в запросах 
* maintenance_work_mem
...
#### память тоже загружена почти на 100%
