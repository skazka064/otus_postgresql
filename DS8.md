...
#### synchronous_commit определяет, после завершения какого уровня обработки WAL сервер будет сообщать об успешном выполнении операции. Допустимые значения remote_apply(применено удаленно), on(включео, по умолчанию), remote_write(записано удаленно), local(локально), off(выключено). Локальное действие всех отличных от off режимов заключается в ожидании локального сброса WAL на диск. В режиме off ожидание отсутствует, и поэтому образуется окно от момента, когда клиент узнает об успешном завершении, до момента, когда транзакция действительно зафиксирует свои результаты на диск. Максимальный размер этого окна равен wal_writer_delay * 3 Однако значение off не угрожает целостности данных. Сбой ОС или БД может привести к потере последних транзакций, считавшихся зафиксированными, но состояние БД будет точно таким же, как и в случае штатного прерывания этих транзакций. Следовательно увеличение TPS произошло за счет отсутствия времени ожижания подтверждения сброса WAL на диск.
